...
cat("B亚家族在数据中:", paste(available, collapse = ", "), "\n")
...
if (length(available) >= 2) {
  ...
  if ("NDUFB7" %in% available && "NDUFB8" %in% available) {
    ...
    } else if (ct$estimate > 0.1 && ct$p.value < 0.1) {
      cat("✅ 趋势性正相关\n")
    }
    ...
    write.csv(data.frame(
      ...
      interpretation = ifelse(ct$estimate > 0.3 && ct$p.value < 0.05, "Co_module_support",
                             ifelse(ct$estimate > 0.1, "Trend_support", "Weak"))
  ...
  if ("NDUFB7" %in% available && "NDUFB8" %in% available) {
    ...
    cat("[保存] 15_NDUFB7_vs_NDUFB8_scatter.pdf\n")
  }
  ...
  pheatmap(avg_expr, scale = "row",
           color = colorRampPalette(c("blue", "white", "red"))(100),
           main = "B-subfamily Expression by Cluster (Z-score)")
  dev.off()
  cat("[保存] 16_B_family_cluster_heatmap.pdf\n")
} else {
  cat("❌ B亚家族成员不足\n")
}
cat("\n🎉 NDUFB7-NDUFB8共表达分析完成！\n\n")
