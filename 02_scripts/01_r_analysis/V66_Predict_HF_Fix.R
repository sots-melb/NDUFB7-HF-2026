...
# 1. 过滤：只保留入院第一天 (Day 1 / admission)
df_day1 <- df[grepl("1st day|admission", df$TimePoint, ignore.case=TRUE), ]

message("▶ 诊断：数据清洗前，Day 1 的 HF Progression 标签分布：")
print(table(df_day1$Progression, useNA="always"))

# 2. 深度清洗：去前缀、转小写、去首尾空格
df_day1$Progression <- gsub("hf progression:", "", df_day1$Progression, ignore.case=TRUE)
df_day1$Progression <- tolower(trimws(df_day1$Progression))

message("▶ 诊断：深度清洗后，Day 1 的 HF Progression 标签分布：")
print(table(df_day1$Progression, useNA="always"))

# 3. 提取有效患者 (剔除 n/a 的健康人)
df_day1 <- df_day1[df_day1$Progression %in% c("yes", "no"), ]

if(nrow(df_day1) == 0) {
    stop("❌ 严重错误：清洗后仍然为 0！请检查上面的诊断输出，看看作者到底填了什么词。")
}

df_day1$Progression <- factor(df_day1$Progression, levels=c("no", "yes"), labels=c("Stable (No HF)", "HF Progression"))
message(sprintf("✅ 完美！成功锁定 %d 名真实心梗患者的入院首日样本！", nrow(df_day1)))

# 4. 统计学检验
res <- wilcox.test(NDUFB7 ~ Progression, data = df_day1)

# 5. Logistic 回归计算风险比 (Odds Ratio)
...
or <- exp(-summary(log_model)$coefficients[2, "Estimate"]) 
p_log <- summary(log_model)$coefficients[2, "Pr(>|z|)"]

message("\n=======================================================")
message("🌟 临床转化闭环：入院首日 NDUFB7 预测心衰恶化结果 🌟")
message(sprintf(" 1. Wilcoxon 组间差异: p = %.4f", res$p.value))
message(sprintf(" 2. Logistic 回归预测: p = %.4f", p_log))
message(sprintf(" 3. 临床风险: NDUFB7 每降低 1 个标准差，心衰恶化风险增加 %.2f 倍！", or))
message("=======================================================")

# 6. 绘制投稿级 Figure 6 箱线图
