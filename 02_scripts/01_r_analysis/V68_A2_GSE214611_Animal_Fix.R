...
suppressMessages(library(Matrix))  # 显式加载
...
for(s in samples) {
  ...
  tryCatch({
    ...
    else if(grepl("snd0", s, ignore.case=TRUE)) time <- "SND0"
    else if(grepl("snd1", s, ignore.case=TRUE)) time <- "SND1"
    else if(grepl("snd3", s, ignore.case=TRUE)) time <- "SND3"
    else if(grepl("snd7", s, ignore.case=TRUE)) time <- "SND7"
    
    results <- rbind(results, data.frame(Sample=s, TimePoint=time, N_Cells=n_cells, NDUFB7_Mean=ndufb7_mean, NDUFB7_Pct=ndufb7_pct, ComplexI_Mean=ci_mean_val, ComplexI_Cor=ci_cor, FTH1_Cor=fth1_cor, stringsAsFactors=FALSE))
  }, error=function(e) message("  ❌ ", s, ": ", conditionMessage(e)))
}

results <- results[order(results$TimePoint, results$Sample), ]
message("▶ 时间序列汇总:")
print(results)

# 按TimePoint聚合
if(nrow(results)>0) {
  agg <- aggregate(cbind(NDUFB7_Mean, NDUFB7_Pct, ComplexI_Cor, FTH1_Cor)~TimePoint, data=results, FUN=function(x) c(Mean=mean(x, na.rm=TRUE), N=sum(!is.na(x))))
  message("▶ 按时间点聚合:")
  print(agg)
}

out <- "~/Projects/NDUFB7_HF_2026_04_20/03_results/02_tables/V68_GSE214611_TimeSeries_Fix.csv"
