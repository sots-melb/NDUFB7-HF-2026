...
message("========================================")
message("V123_ADD: Fig 4机制整合（修复dplyr依赖）")
message("========================================")
...
df_mono <- data.frame(pseudotime = pt, NDUFB7 = expr) %>% 
...
pA <- ggplot(df_mono, aes(x = pseudotime, y = NDUFB7)) +
  ...
  annotate("text", x = breakpoint + 0.3, y = max(df_mono$NDUFB7, na.rm = TRUE)*0.95, label = "Breakpoint\np=4.3×10⁻⁴", color = "red", hjust = 0, size = 2.8) +
...
pB <- ggplot(t27, aes(x = Complex, y = Log2FC, fill = sig)) +
  ...
  geom_text(aes(label = ifelse(P_Value < 0.05, format(P_Value, digits=1, scientific=TRUE), "NS")), 
            vjust = ifelse(t27$Log2FC > 0, -0.5, 1.2), size = 2.5) +
  scale_fill_manual(values = c("Significant" = "#FDE725", "NS" = "grey70")) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  labs(title = "B. OXPHOS Multi-Complex Collapse", x = "Complex", y = "Log2FC") +
  theme_minimal(base_size = 9) + theme(legend.position = "none")
...
pC <- ggplot(ferro, aes(x = NDUFB7_group, y = ACSL4_GPX4_ratio, fill = NDUFB7_group)) +
  ...
  labs(title = "C. Ferroptosis Susceptibility", subtitle = "ACSL4/GPX4 ratio", x = "NDUFB7 Level", y = "Ratio") +
  theme_minimal(base_size = 9) + theme(legend.position = "none")

# --- 面板D: 恶性循环概念图（ggplot模拟） ---
pD <- ggplot() + xlim(0, 10) + ylim(0, 10) + theme_void() +
  ...
  annotate("segment", x = 6.5, xend = 6, y = 3.5, yend = 3.5, arrow = arrow(length = unit(0.2, "cm")), linewidth = 1) +
  annotate("segment", x = 3.5, xend = 3.5, y = 6, yend = 6.5, arrow = arrow(length = unit(0.2, "cm")), linewidth = 1, color = "red", linetype = "dashed") +
  annotate("text", x = 5, y = 2, label = "Vicious Cycle: Stress → NDUFB7↓ → OXPHOS↓ → Ferroptosis Susceptibility → Partial Rescue", size = 3.2, fontface = "italic") +
