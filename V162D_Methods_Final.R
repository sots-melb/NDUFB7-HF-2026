...
outdir <- file.path(PROJECT, "03_results/V162D_Methods_Final")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

methods <- '### Methods (Key Sections)

**Single-cell bimodality testing.** Because conventional Gaussian mixture modeling (GMM) is unreliable under extreme zero-inflation, we implemented a dual-evidence framework. First, zero-inflated GMM (ZI-GMM) was fit with mclust (G=1–3, modelNames="V") and BIC/AIC recorded. Second, the Hartigans\' dip test (diptest package, B=2000 permutations) was used as a non-parametric alternative that makes no distributional assumptions. Bimodality was claimed only when (i) Dip test rejected unimodality (p<0.05) and (ii) kernel density estimates showed a visible shoulder/secondary peak. This conservative approach avoids false-positive GMM claims in zero-inflated scRNA-seq data.
...
cat(methods, file = file.path(outdir, "V162D_Methods_Key.md"))
message("[DONE] V162D: ", outdir)
